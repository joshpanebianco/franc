class Job < ActiveRecord::Base
  ...
  scope :keyword_search, ->(search) {where('lower(company) LIKE ? OR lower(description) LIKE ? OR lower(requirements) LIKE ? OR lower(location) LIKE ? OR lower(position) LIKE ? OR lower(contract) LIKE ?', "%#{search}%", "%#{search}%", "%#{search}%", "%#{search}%", "%#{search}%", "%#{search}%" ) if !search.blank?}
